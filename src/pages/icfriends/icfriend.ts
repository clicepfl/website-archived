...
import data from './icfriend-list.json'
import { logger } from '../../logger'
import { fstat } from 'fs'

/**
 * Icfriend content path
 * Must correct to src/ folder since this will be compiled in dist
 */
const CONTENT_PATH = path.resolve(__dirname, "../../../src/pages/icfriends/assets/")

class IcfriendMember {
    name: string
    imageURL: string
    websiteURL?: string
    role: string
    ...
    /**
     * Builds a IcfriendMember from any Object that has the correct shape
     *
     * @param   data    Object to convert to IcfriendMember
     ...
     */
    static fromAny(data: any): IcfriendMember {
        if (!Icfriend.is(data)) {
            const message = `Cannot cast data to type IcfriendMember : wrong shape ${JSON.stringify(data)}`
            logger.log(message)
            throw new Error(message)
        } else {
            return new IcfriendMember(data.name, data.imageURL, data.websiteURL, data.role)
        }
    }
     ...
     *
     * @param data Object to check if of type IcfriendMember
     * @warning Modify this method if the properties of the class are changed!
     */
...
class Icfriend {
    ...
    pageBody: string
    catchPhrase: string
    members: IcfriendMember[]
    social: { string: string }

    constructor(name: string, slug: string, imageURL: string, shortDescription: string, pageBody: string, catchPhrase: string, members: IcfriendMember[], social: { string: string }) {
        this.name = name
        this.slug = slug
        this.imageURL = imageURL
    ...
    static fromAny(data: any): Icfriend {
        if (!Icfriend.is(data)) {
            ...
            return new Icfriend(data.name, data.slug, data.imageURL, data.shortDescription, data.pageBody, data.catchPhrase, data.members, data.social)
        }
    }
    ...
    static is(data: any): boolean {
        return (
            ...
            data.members !== undefined && Array.isArray(data.members) && data.members.every((e: any) => IcfriendMember.is(e)) &&
...
class IcfriendComponent {
    private readonly icfriendList: Icfriend[] = data.map(jsonData => {
        ...
        if (!fs.existsSync(pageBodyMdPath)) {
            const msg = `Markdown body file missing for icfriend ${builder.name} at ${pageBodyMdPath}`
            logger.log(msg)
            throw new Error(msg)
        }
        ...
        return Icfriend.fromAny(builder)
    })

    list(): Icfriend[] {
        return Object.assign([], this.icfriendList)
    }
}

const icfriendComponent = new IcfriendComponent()
export { Icfriend, icfriendComponent }
